...
Define the screen reader flow standard for DocuPress CMS so VuePress content and Netlify CMS editorial changes remain secure, reviewable, and deployable.
...
- The screen reader flow requirement is explicit and testable.
...
Keep implementation work focused on one screen reader flow outcome and note any Netlify operator action in the pull request.