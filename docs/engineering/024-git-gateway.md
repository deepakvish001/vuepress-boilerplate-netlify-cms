...
Define the git gateway standard for DocuPress CMS so VuePress content and Netlify CMS editorial changes remain secure, reviewable, and deployable.
...
- The git gateway requirement is explicit and testable.
...
Keep implementation work focused on one git gateway outcome and note any Netlify operator action in the pull request.